...
function parse_scope(array $tokens, int &$index): ScopeNode {
  ...
  while ($index < count($tokens)) {
    ...
    while ($index < count($tokens)) {
      ...
      # if we got a open bracket, we parse a new scope
      if ($token->type === TokenType::OPEN_BRACKET) {
        $ast_node->children[] = parse_scope($tokens, $index);
        continue;
      }
      ...
      if ($token->type === TokenType::IDENTIFIER) {
        ...
        if (
          ...
          if (
            ...
            $ast_node->children[] = $function_node;
        ...
        ######################################################
        # check if thefield is defined with a single token
        # f.e. my_field: Int
